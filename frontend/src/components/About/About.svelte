...
<div class="space-y-4">
  ...
  <div class="bg-white rounded-xl border border-gray-100 p-5">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-2.5">
        <div class="w-6 h-6 rounded-md bg-gradient-to-br from-gray-900 to-gray-700 flex items-center justify-center">
        ...
        <h2 class="text-[13px] font-semibold text-gray-900">{t.ecosystemTitle || 'WgpSec Infra Ecosystem'}</h2>
      </div>
      <span class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Architecture</span>
    </div>

    <!-- Ecosystem Introduction -->
    <p class="text-[12px] text-gray-600 leading-relaxed mb-4">{t.ecosystemIntro || 'WgpSec Infra Ecosystem 将红队基础设施的完整生命周期拆分为模板定义、资源编排、环境装配三层，由 RedC 作为统一控制面串联各组件，实现从场景选择到云端交付的全自动化闭环。'}</p>

    <!-- Pipeline Flow Visualization -->
    <div class="relative bg-gradient-to-r from-gray-50/80 via-gray-50/40 to-gray-50/80 rounded-lg border border-gray-100/80 p-4 mb-4">
      <div class="grid grid-cols-4 gap-0 items-center">
        <!-- Node: redc-template -->
        <div class="flex flex-col items-center">
          <button
            class="group relative w-full max-w-[130px] cursor-pointer"
            onclick={() => openLink('https://github.com/wgpsec/redc-template')}
          >
            <div class="flex flex-col items-center gap-1.5 p-2.5 rounded-lg border border-amber-200/60 bg-gradient-to-b from-amber-50 to-white hover:border-amber-300 hover:shadow-sm transition-all">
              <div class="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center">
                <svg class="w-4 h-4 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
              </div>
              <span class="text-[11px] font-semibold text-gray-800">redc-template</span>
              <span class="text-[9px] text-gray-400">{t.ecosystemTemplateRole || '场景仓库'}</span>
            </div>
          </button>
        </div>

        <!-- Node: RedC (highlighted as current) -->
        <div class="flex flex-col items-center">
          <button
            class="group relative w-full max-w-[130px] cursor-pointer"
            onclick={() => openLink('https://github.com/wgpsec/redc')}
          >
            <div class="flex flex-col items-center gap-1.5 p-2.5 rounded-lg border-2 border-red-300/80 bg-gradient-to-b from-red-50 to-white shadow-sm hover:shadow-md hover:border-red-400 transition-all ring-2 ring-red-100/50">
              <div class="absolute -top-1.5 -right-1.5 w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
              <div class="w-8 h-8 rounded-lg bg-red-100 flex items-center justify-center">
                <span class="text-sm font-bold text-red-600">R</span>
              </div>
              <span class="text-[11px] font-bold text-gray-900">RedC</span>
              <span class="text-[9px] text-red-500 font-medium">{t.ecosystemRedcRole || '控制面'}</span>
            </div>
          </button>
        </div>
        ...
        <div class="flex flex-col items-center">
          <button
            class="group relative w-full max-w-[130px] cursor-pointer"
            onclick={() => openLink('https://github.com/ffffffff0x/f8x')}
          >
            <div class="flex flex-col items-center gap-1.5 p-2.5 rounded-lg border border-cyan-200/60 bg-gradient-to-b from-cyan-50 to-white hover:border-cyan-300 hover:shadow-sm transition-all">
              <div class="w-8 h-8 rounded-lg bg-cyan-100 flex items-center justify-center">
                <svg class="w-4 h-4 text-cyan-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17l-5.384 3.183A2.25 2.25 0 013 16.268V7.732a2.25 2.25 0 013.036-2.085l5.384 3.183a2.25 2.25 0 010 3.852zm0-6.34V3.787a2.25 2.25 0 013.036-2.085l5.384 3.183a2.25 2.25 0 010 3.852l-5.384 3.183a2.25 2.25 0 01-3.036-2.085V8.83z" /></svg>
              </div>
              <span class="text-[11px] font-semibold text-gray-800">f8x</span>
              <span class="text-[9px] text-gray-400">{t.ecosystemF8xRole || '装配引擎'}</span>
            </div>
          </button>
        </div>
        ...
        <!-- Node: Multi-Cloud -->
        <div class="flex flex-col items-center">
          <div class="flex flex-col items-center gap-1.5 p-2.5 rounded-lg border border-gray-200/60 bg-gradient-to-b from-gray-50 to-white">
            <div class="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center">
              <svg class="w-4 h-4 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15a4.5 4.5 0 004.5 4.5H18a3.75 3.75 0 001.332-7.257 3 3 0 00-3.758-3.848 5.25 5.25 0 00-10.233 2.33A4.502 4.502 0 002.25 15z" /></svg>
            </div>
            <span class="text-[11px] font-semibold text-gray-800">{t.ecosystemCloudNode || 'Multi-Cloud'}</span>
            <span class="text-[9px] text-gray-400">AWS / Ali / TC</span>
          </div>
        </div>
      </div>

      <!-- Flow arrows between nodes -->
      <div class="absolute inset-0 pointer-events-none" style="top: 50%; transform: translateY(-50%);">
        <div class="grid grid-cols-4 gap-0 h-0 items-center px-4">
          <div class="flex justify-end pr-0">
            <div class="w-4 flex items-center justify-center">
              <svg class="w-4 h-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>
            </div>
          </div>
          <div class="flex justify-end pr-0">
            <div class="w-4 flex items-center justify-center">
    ...
    <!-- Sub-capabilities under RedC -->
    <div class="grid grid-cols-3 gap-2 mb-4">
      <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50/80 border border-gray-100/80">
        <div class="w-1.5 h-1.5 rounded-full bg-emerald-400"></div>
        <span class="text-[10px] text-gray-600 font-medium">Terraform IaC</span>
      </div>
      <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50/80 border border-gray-100/80">
        <div class="w-1.5 h-1.5 rounded-full bg-violet-400"></div>
        <span class="text-[10px] text-gray-600 font-medium">MCP Bridge</span>
      </div>
      <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50/80 border border-gray-100/80">
        <div class="w-1.5 h-1.5 rounded-full bg-blue-400"></div>
        <span class="text-[10px] text-gray-600 font-medium">Compose</span>
      </div>
    </div>

    <!-- Ecosystem description cards -->
    <div class="grid grid-cols-3 gap-2.5">
      {#each ecosystem as eco, i}
        <button
          class="group relative text-left p-3 rounded-lg border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all cursor-pointer bg-white"
          onclick={() => openLink(eco.url)}
        >
          <div class="flex items-center gap-2 mb-1.5">
            <div class="w-5 h-5 rounded {eco.color} flex items-center justify-center flex-shrink-0">
              {#if i === 0}
                <span class="text-[10px] font-bold text-red-500">R</span>
              {:else if i === 1}
                <svg class="w-3 h-3 {eco.iconColor}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
              {:else}
                <svg class="w-3 h-3 {eco.iconColor}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>
              {/if}
            </div>
            <span class="text-[11px] font-semibold text-gray-800">{t[eco.nameKey] || eco.name}</span>
            <svg class="w-3 h-3 text-gray-300 group-hover:text-gray-500 transition-colors ml-auto opacity-0 group-hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 19.5l15-15m0 0H8.25m11.25 0v11.25" /></svg>
          </div>
          <p class="text-[10px] text-gray-500 leading-relaxed">{t[eco.descKey] || eco.descFallback}</p>
        </button>
      {/each}
    </div>

    <!-- Footer flow description -->
    <div class="mt-3 pt-3 border-t border-gray-100/80 flex items-center gap-2">
      <svg class="w-3 h-3 text-gray-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" /></svg>
      <p class="text-[10px] text-gray-400 leading-relaxed">{t.ecosystemFlow || 'Templates → RedC orchestration → f8x provisioning → Cloud resources — fully automated'}</p>
    </div>
  </div>
